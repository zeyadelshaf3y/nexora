...
- **Drag to close**: Opt-in via `dragToClose: true` (or config) on open. Use `dragFrom: 'handle'` (default) with `nxrDrawerDragHandle` on a handle element — recommended for touch and scrollable content. Optional `dragFrom: 'pane'` drags from anywhere on the drawer except interactive elements (buttons, links, inputs); best for pointer/mouse on non-scrollable drawers. Dismiss axis follows placement (and RTL for start/end). Closes with reason `'gesture'`. Escape, backdrop, and close button still work. Handle edge is exposed as `data-nxr-drawer-drag-edge` (`block-end` | `block-start` | `inline-end` | `inline-start`). Reposition and backdrop dismiss are suppressed during an active drag.
...
- **Popup coordination (default on)**: When an anchored popup (popover, menu, select, combobox) opens on the **same trigger element**, the tooltip closes immediately and does not reopen via hover or focus while that popup is open. Opt out per instance with `nxrTooltipCloseOnPopup="false"` or globally via `TOOLTIP_DEFAULTS_CONFIG` (`closeOnPopup: false`). Coordination is keyed by element identity; external-anchor popovers on a different element than the tooltip host are not coordinated.
