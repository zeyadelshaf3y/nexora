...
describe('createAnchoredOverlayConfig', () => {
  ...
  it('includes optional backdrop class and style when provided', () => {
    const config = createAnchoredOverlayConfig({
      anchor,
      placement: 'bottom',
      offset: 8,
      ...
      focusStrategy: new NoopFocusStrategy(),
      closeAnimationDurationMs: 150,
      backdropClass: ['nxr-popover-backdrop', 'custom-backdrop'],
      backdropStyle: { backdropFilter: 'blur(2px)' },
    });

    expect(config.backdropClass).toEqual(['nxr-popover-backdrop', 'custom-backdrop']);
    expect(config.backdropStyle).toEqual({ backdropFilter: 'blur(2px)' });
  });
