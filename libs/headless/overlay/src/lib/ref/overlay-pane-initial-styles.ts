...
export function applyInitialOverlayPaneAndBackdropStyles(
  ...
  if (backdrop) {
    ...
    addClasses(backdrop, config.nxrBackdropClass);
    applyStyles(backdrop, mergeBackdropStyleInputs(config.backdropStyle, config.nxrBackdropStyles));
...
function mergeBackdropStyleInputs(
  backdropStyle: Record<string, string> | undefined,
  nxrBackdropStyles: Record<string, string> | undefined,
): Record<string, string> | undefined {
  if (backdropStyle == null) return nxrBackdropStyles;
  if (nxrBackdropStyles == null) return backdropStyle;

  return { ...backdropStyle, ...nxrBackdropStyles };
}
