...
import {
  ComboboxBackdropDefaultsPrecedenceHostComponent,
...
describe('ComboboxComponent', () => {
  ...
  it('applies backdrop class precedence (overlay defaults < combobox defaults < instance inputs)', async () => {
    const precedenceFixture = TestBed.createComponent(
      ComboboxBackdropDefaultsPrecedenceHostComponent,
    );
    const host = precedenceFixture.componentInstance;
    precedenceFixture.detectChanges();

    expect(await host.comboRef().open()).toBe(true);
    await settleAfterStable(precedenceFixture);

    const backdrop = document.querySelector('.nxr-combobox-backdrop') as HTMLElement | null;
    expect(backdrop).not.toBeNull();
    expect(backdrop?.classList.contains('defaults-combobox-backdrop')).toBe(true);
    expect(backdrop?.classList.contains('instance-combobox-backdrop')).toBe(true);
    expect(backdrop?.classList.contains('defaults-overlay-nxr-backdrop')).toBe(true);
    expect(backdrop?.classList.contains('instance-combobox-nxr-backdrop')).toBe(true);
  });
});
