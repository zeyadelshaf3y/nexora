...
@Component({
  ...
  providers: [
    ...
    provideMenuDefaults({
      backdropClass: 'defaults-menu-backdrop',
    }),
  ],
  template: `
    <nxr-menu
      [hasBackdrop]="true"
      backdropClass="instance-menu-backdrop"
      nxrBackdropClass="instance-menu-nxr-backdrop"
      [closeAnimationDurationMs]="0"
      #m="nxrMenu"
    >
      <button nxrMenuTrigger>Actions</button>
...
class MenuBackdropDefaultsPrecedenceHost {
  readonly item1 = { id: 1, label: 'Item 1' };
  readonly menuRef = viewChild.required<MenuComponent>('m');
}
...
describe('MenuComponent', () => {
  describe('basic open/close', () => {
    ...
    it('applies backdrop class precedence (overlay defaults < menu defaults < instance inputs)', async () => {
      await TestBed.configureTestingModule({
        imports: [MenuBackdropDefaultsPrecedenceHost],
      }).compileComponents();

      const fixture = TestBed.createComponent(MenuBackdropDefaultsPrecedenceHost);
      fixture.detectChanges();
      await openMenu(fixture);

      const backdrop = document.querySelector('.nxr-menu-backdrop') as HTMLElement | null;
      expect(backdrop).not.toBeNull();
      expect(backdrop?.classList.contains('defaults-menu-backdrop')).toBe(true);
      expect(backdrop?.classList.contains('instance-menu-backdrop')).toBe(true);
      expect(backdrop?.classList.contains('defaults-overlay-nxr-backdrop')).toBe(true);
      expect(backdrop?.classList.contains('instance-menu-nxr-backdrop')).toBe(true);
    });
  });
