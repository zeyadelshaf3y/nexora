...
describe('buildSelectDropdownRefOptions', () => {
  ...
  it('merges select backdrop base class with user backdropClass', () => {
    const opts = buildSelectDropdownRefOptions(
      minimalInput({ backdropClass: () => ['app-select-backdrop'] }),
    );
    expect(opts.backdropClass?.()).toEqual([SELECT_BACKDROP_CLASS, 'app-select-backdrop']);
  });
