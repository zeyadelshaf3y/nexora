...
describe('mergeDropdownBackdropClasses', () => {
  it('returns only base when user class is undefined', () => {
    expect(mergeDropdownBackdropClasses('nxr-base', undefined)).toEqual(['nxr-base']);
  });

  it('returns only base when user class is empty string', () => {
    expect(mergeDropdownBackdropClasses('nxr-base', '')).toEqual(['nxr-base']);
  });

  it('appends a string user class', () => {
    expect(mergeDropdownBackdropClasses('nxr-base', 'user')).toEqual(['nxr-base', 'user']);
  });

  it('appends array user classes', () => {
    expect(mergeDropdownBackdropClasses('nxr-base', ['a', 'b'])).toEqual(['nxr-base', 'a', 'b']);
  });
});
